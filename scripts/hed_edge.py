...
arguments_strIn = '../crop_image/'
arguments_strOut = '../edge_image/'
...
if __name__ == '__main__':
	count = 1
	for image_path in os.listdir(arguments_strIn):
		print("processing %d image..." % count)
		count += 1
		input_path = os.path.join(arguments_strIn, image_path)
		tensorInput = torch.FloatTensor(numpy.array(PIL.Image.open(input_path))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0))
		tensorOutput = estimate(tensorInput)
		PIL.Image.fromarray((tensorOutput.clamp(0.0, 1.0).numpy().transpose(1, 2, 0)[:, :, 0] * 255.0).astype(numpy.uint8)).save(os.path.join(arguments_strOut, image_path))
